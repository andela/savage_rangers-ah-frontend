...
export default {
  ...
  getRandomArticles: () => (dispatch) => {
    axios
      .get('/api/articles?offset=5&&limit=5')
  ...
  getRecentArticles: () => (dispatch) => {
    axios
      .get('/api/articles?limit=5')
      .then(res => dispatch({ type: GET_RECENT_ARTICLES, payload: res.data.result.Articles }))
      .catch(error => dispatch({ type: GET_RECENT_ARTICLES_ERROR, payload: error }));
  }
};
