...
describe('<articles />', () => {
  describe('render()', () => {
    it('should display the right rating', () => {
      const statistics = [
    ...
    it('it should render a single article', () => {
      ...
      const article = mount(<MemoryRouter><Article store={store} {...props} /></MemoryRouter>);
      expect(article.find('Router').exists()).toBe(true);
      expect(article.find('Article').exists()).toBe(true);
      article.instance().render();
    });

    it('it should render a single with article deferent data', () => {
      const get = jest.fn(() => { });
      const props = {
        getArticles: get,
        article: { articles: [{}] },
